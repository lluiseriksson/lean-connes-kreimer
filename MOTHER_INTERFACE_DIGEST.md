...
monomials.  The
wrapper `ConnesKreimer.AdmissibleCutData.graftOperator_zero` exposes the same
supplied operator on zero.  The
wrapper
...
`ConnesKreimer.AdmissibleCutData.graftOperator_zero` and
...
`ConnesKreimer.AdmissibleCutData.graftOperator_zero` rewrites the same supplied
operator on zero to zero.  The wrapper
